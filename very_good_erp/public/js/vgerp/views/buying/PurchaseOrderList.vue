...
const STATUS = {
	...
	"To Receive and Bill": { bg: "#FFFBEB", color: "#B45309" },
	"To Bill":             { bg: "#FFF7ED", color: "#C2410C" },
	"To Receive":          { bg: "#EFF6FF", color: "#1D4ED8" },
...
async function load() {
	...
	try {
		...
		rows.value = await getList("Purchase Order", {
...
function openDoc(name) { window.open(`/app/purchase-order/${name}`, "_blank"); }
function createNew() { window.open("/app/purchase-order/new-purchase-order-1", "_blank"); }
...
<template>
	<div class="p-8">
		<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;">
			<div>
				<h1 style="font-family:'DM Serif Display',Georgia,serif;font-size:24px;color:#1A1714;letter-spacing:-.02em;margin:0 0 3px;">Purchase Orders</h1>
				<p style="font-size:12.5px;color:#9C9086;margin:0;">Buying &rsaquo; Purchase Orders</p>
			</div>
			...
			<table style="width:100%;border-collapse:collapse;">
				<thead>
					<tr style="background:#F7F4EF;border-bottom:1px solid #E5E0D6;">
						<th v-for="h in ['Order #','Supplier','Order Date','Expected','Total','Status']" :key="h"
				...
				<tbody>
					...
					<tr v-else v-for="row in rows" :key="row.name" @click="openDoc(row.name)"
						...
						<td style="padding:10px 16px;color:#6B6459;font-size:13px;font-family:'DM Mono',monospace;">{{ row.schedule_date || "—" }}</td>
