...
async function load() {
	...
	error.value = null;
	try {
		const filters = search.value ? [["supplier_name", "like", `%${search.value}%`]] : [];
		rows.value = await getList("Supplier", {
			fields: ["name", "supplier_name", "supplier_type", "supplier_group", "country", "disabled"],
			filters,
			limit: PAGE_SIZE,
...
function openDoc(name) { window.open(`/app/supplier/${name}`, "_blank"); }
function createNew() { window.open("/app/supplier/new-supplier-1", "_blank"); }
...
<template>
	<div class="p-8">
		<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;">
			<div>
				<h1 style="font-family:'DM Serif Display',Georgia,serif;font-size:24px;color:#1A1714;letter-spacing:-.02em;margin:0 0 3px;">Suppliers</h1>
				<p style="font-size:12.5px;color:#9C9086;margin:0;">Buying &rsaquo; Suppliers</p>
			</div>
		...
		<input v-model="search" placeholder="Search suppliers…"
			...
			<table style="width:100%;border-collapse:collapse;">
				<thead>
					<tr style="background:#F7F4EF;border-bottom:1px solid #E5E0D6;">
						<th v-for="h in ['ID','Name','Type','Group','Country','Status']" :key="h"
				...
				<tbody>
					...
					<tr v-else-if="!rows.length"><td colspan="6" style="padding:32px;text-align:center;color:#9C9086;font-size:13px;">No suppliers found</td></tr>
					<tr v-else v-for="row in rows" :key="row.name" @click="openDoc(row.name)"
						...
						<td style="padding:10px 16px;color:#3D3530;font-size:13px;">{{ row.supplier_name }}</td>
						<td style="padding:10px 16px;color:#6B6459;font-size:13px;">{{ row.supplier_type }}</td>
						<td style="padding:10px 16px;color:#6B6459;font-size:13px;">{{ row.supplier_group }}</td>
						<td style="padding:10px 16px;color:#6B6459;font-size:13px;">{{ row.country || "—" }}</td>
