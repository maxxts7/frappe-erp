...
const TYPE_COLORS = {
	...
	"Pay":               { bg: "#FEF2F2", color: "#B91C1C" },
	"Internal Transfer": { bg: "#EFF6FF", color: "#1D4ED8" },
};
...
async function load() {
	...
	try {
		...
		rows.value = await getList("Payment Entry", {
			fields: ["name", "party_type", "party", "posting_date", "paid_amount", "payment_type", "mode_of_payment"],
			filters,
			limit: PAGE_SIZE,
...
function openDoc(name) { window.open(`/app/payment-entry/${name}`, "_blank"); }
function createNew() { window.open("/app/payment-entry/new-payment-entry-1", "_blank"); }
...
<template>
	<div class="p-8">
		<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;">
			<div>
				<h1 style="font-family:'DM Serif Display',Georgia,serif;font-size:24px;color:#1A1714;letter-spacing:-.02em;margin:0 0 3px;">Payments</h1>
				<p style="font-size:12.5px;color:#9C9086;margin:0;">Accounts &rsaquo; Payments</p>
			</div>
		...
		<input v-model="search" placeholder="Search payments…"
			...
			<table style="width:100%;border-collapse:collapse;">
				<thead>
					<tr style="background:#F7F4EF;border-bottom:1px solid #E5E0D6;">
						<th v-for="h in ['Payment #','Party','Date','Amount','Mode','Type']" :key="h"
				...
				<tbody>
					...
					<tr v-else-if="!rows.length"><td colspan="6" style="padding:32px;text-align:center;color:#9C9086;font-size:13px;">No payments found</td></tr>
					<tr v-else v-for="row in rows" :key="row.name" @click="openDoc(row.name)"
						...
						<td style="padding:10px 16px;color:#3D3530;font-size:13px;">{{ row.party || "—" }}</td>
						<td style="padding:10px 16px;color:#6B6459;font-size:13px;font-family:'DM Mono',monospace;">{{ row.posting_date }}</td>
						<td style="padding:10px 16px;color:#1A1714;font-size:13px;font-family:'DM Mono',monospace;font-weight:500;">
							{{ row.paid_amount?.toLocaleString("en-US", { minimumFractionDigits: 2 }) }}
						</td>
						<td style="padding:10px 16px;color:#6B6459;font-size:13px;">{{ row.mode_of_payment || "—" }}</td>
						<td style="padding:10px 16px;">
							<span style="font-family:'DM Mono',monospace;font-size:10.5px;font-weight:500;letter-spacing:.03em;border-radius:20px;padding:3px 9px;"
								:style="{ background: (TYPE_COLORS[row.payment_type]||{bg:'#F5F5F5'}).bg, color: (TYPE_COLORS[row.payment_type]||{color:'#6B7280'}).color }">
								{{ row.payment_type }}
							</span>
						</td>
					</tr>
