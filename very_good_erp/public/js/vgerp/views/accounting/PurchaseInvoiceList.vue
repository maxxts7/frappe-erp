...
function openDoc(name) { window.open(`/app/purchase-invoice/${name}`, "_blank"); }
function createNew() { window.open("/app/purchase-invoice/new-purchase-invoice-1", "_blank"); }
		...
		<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;">
			<div>
				<h1 style="font-family:'DM Serif Display',Georgia,serif;font-size:24px;color:#1A1714;
				           letter-spacing:-.02em;margin:0 0 3px;">Purchase Invoices</h1>
				<p style="font-size:12.5px;color:#9C9086;margin:0;">Accounts &rsaquo; Purchase Invoices</p>
			</div>
			...
			<table style="width:100%;border-collapse:collapse;">
				<thead>
					<tr style="background:#F7F4EF;border-bottom:1px solid #E5E0D6;">
						<th v-for="h in ['Invoice #','Supplier','Date','Total','Outstanding','Status']" :key="h"
							style="padding:10px 16px;text-align:left;font-family:'DM Mono',monospace;
							       font-size:9.5px;letter-spacing:.12em;text-transform:uppercase;color:#6B6459;font-weight:500;">{{ h }}</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr v-else-if="!rows.length"><td colspan="6" style="padding:32px;text-align:center;color:#9C9086;font-size:13px;">No invoices found</td></tr>
					<tr v-else v-for="row in rows" :key="row.name" @click="openDoc(row.name)"
						...
						<td style="padding:10px 16px;font-size:13px;font-family:'DM Mono',monospace;"
							:style="{ color: row.outstanding_amount > 0 ? '#B91C1C' : '#15803D' }">
