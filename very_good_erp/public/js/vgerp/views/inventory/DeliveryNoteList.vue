...
const STATUS = {
	...
	"Return Issued": { bg: "#F5F0FF", color: "#7C3AED" },
};
...
async function load() {
	...
	try {
		...
		rows.value = await getList("Delivery Note", {
			fields: ["name", "customer", "posting_date", "grand_total", "status", "lr_no"],
			filters,
			limit: PAGE_SIZE,
...
function openDoc(name) { window.open(`/app/delivery-note/${name}`, "_blank"); }
function createNew() { window.open("/app/delivery-note/new-delivery-note-1", "_blank"); }
...
<template>
	<div class="p-8">
		<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;">
			<div>
				<h1 style="font-family:'DM Serif Display',Georgia,serif;font-size:24px;color:#1A1714;letter-spacing:-.02em;margin:0 0 3px;">Delivery Notes</h1>
				<p style="font-size:12.5px;color:#9C9086;margin:0;">Inventory &rsaquo; Delivery Notes</p>
			</div>
		...
		<input v-model="search" placeholder="Search delivery notes…"
			...
			<table style="width:100%;border-collapse:collapse;">
				<thead>
					<tr style="background:#F7F4EF;border-bottom:1px solid #E5E0D6;">
						<th v-for="h in ['DN #','Customer','Date','Amount','LR No.','Status']" :key="h"
				...
				<tbody>
					...
					<tr v-else-if="!rows.length"><td colspan="6" style="padding:32px;text-align:center;color:#9C9086;font-size:13px;">No delivery notes found</td></tr>
					<tr v-else v-for="row in rows" :key="row.name" @click="openDoc(row.name)"
						...
						<td style="padding:10px 16px;color:#6B6459;font-size:13px;">{{ row.lr_no || "—" }}</td>
